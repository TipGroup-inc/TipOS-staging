...
//     se held > REPEAT_DELAY (500ms) e (timer_ticks - last_repeat_tick) >= REPEAT_RATE (33Hz):
...
### 7.5 keyboard_read() — Bloqueante com Polling

```c
char keyboard_read(void) {
    while (1) {
        if (kb_head != kb_tail)     // caractere no buffer?
            return kb_buffer[...];  // sim, retorna
        if (inb(0x64) & 0x01)       // hardware tem dado?
            process_scancode(inb(0x60)); // lê e processa
...
| `cd` | `cmd_cd()` | Muda diretório |
| `pwd` | `cmd_pwd()` | Caminho atual |
| `mv` | `cmd_mv()` | Renomeia |
| `cp` | `cmd_cp()` | Copia |
| `rmdir` | `cmd_rmdir()` | Remove diretório |
| `stat` | `cmd_stat()` | Info arquivo |
