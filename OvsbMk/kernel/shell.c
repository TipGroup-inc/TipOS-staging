...
void shell_init(void) {
    ...
    uint32_t desktop_size;
    uint8_t desktop_attr;
    if (fat32_stat("DISP", &desktop_size, &desktop_attr, 0, 0) == 0) {
        cmd_exec("DISP");
    } else {
        console_write("Desktop: DISP nao instalado em /BIN\n");
        console_write("Instale o compositor do repo ../disp como /BIN/DISP\n");
        console_write("e instale o terminal ../term como /BIN/ZIGTERM\n");
        prompt();
    }
}
