// moe moe kyun <3
// moe moe kyun <3
// Linux-compatible syscall dispatcher for OvsbOS (GPL v2)
// ~~ tradutor oficial Linux → OvsbOS ~~ n mexe sem saber oq ta fazendo <3
...
// Se liga, essas são as funções exclusivas do OvsbOS (sou especial~)
pub const ovsbos = struct {
    pub const kbhit = 198;
...
extern fn process_current_uses_linux_abi() c_int;

// ~~ linux_to_ovsbos ~~
// Tabela de tradução de syscalls Linux → OvsbOS.
// Inicializa como identity mapping (cada número mapeia pra si mesmo),
// depois sobrescreve com os mapeamentos específicos.
// Por exemplo: Linux read=0 → OvsbOS read=3, write=1 → OvsbOS write=4...
// Assim programas Linux rodam sem recompilação (mágica~) ☆
const linux_to_ovsbos: [512]u16 = brk: {
    ...
    map[14]  = 14;    // rt_sigprocmask (livre no OvsbOS)
    map[15]  = 15;    // rt_sigreturn (livre no OvsbOS)
    map[131] = 131;   // sigaltstack (livre no OvsbOS)
    map[157] = 157;   // prctl (livre no OvsbOS)
    map[202] = 96;    // futex (202 colide c/ mouse_read) → 96 livre
    map[234] = 103;   // tgkill (234 colide c/ nanosleep) → 103 livre
    map[334] = 334;   // rseq (livre no OvsbOS)
    map[16]  = 54;    // ioctl (stub)
    map[21]  = 33;    // access
    map[22]  = 22;    // pipe (livre no OvsbOS)
    map[23]  = 23;    // select (OvsbOS 23 = select)
    map[24]  = 90;    // sched_yield (24 colide c/ getuid)
    map[25]  = 75;    // mremap (stub ENOSYS)
    map[26]  = 26;    // msync (no-op)
    map[27]  = 27;    // mincore (livre no OvsbOS)
    map[28]  = 28;    // madvise (no-op)
    map[33]  = 91;    // dup2 (33 colide c/ access)
    map[35]  = 234;   // nanosleep (stub)
    map[39]  = 20;    // getpid
    map[40]  = 40;    // sendfile (livre no OvsbOS)
    map[41]  = 41;    // socket (livre no OvsbOS)
    map[42]  = 42;    // connect (livre no OvsbOS)
    map[43]  = 43;    // accept (livre no OvsbOS)
    map[44]  = 44;    // sendto (livre no OvsbOS)
    map[45]  = 45;    // recvfrom (livre no OvsbOS)
    map[46]  = 46;    // sendmsg (livre no OvsbOS)
    map[47]  = 92;    // recvmsg (47 colide c/ getgid)
    map[48]  = 408;   // shutdown (48 colide c/ getegid) → 408 livre
    map[49]  = 49;    // bind (livre no OvsbOS)
    map[50]  = 50;    // listen (livre no OvsbOS)
    map[53]  = 53;    // socketpair (livre no OvsbOS)
    map[54]  = 93;    // fcntl (54 colide c/ ioctl)
    map[57]  = 210;   // fork via spawn
    map[59]  = 208;   // execve
    map[60]  = 1;     // exit
    map[61]  = 61;    // wait4 (61 livre no OvsbOS) — getdents é 78, não 61!
    map[62]  = 62;    // kill
    map[72]  = 72;    // fcntl
    ...
    map[98]  = 98;    // getrusage
    map[100] = 100;   // times
    map[95]  = 400;   // umask (95 colide c/ tkill) → 400 livre no OvsbOS
    map[99]  = 401;   // sysinfo (99 colide c/ sched_setaffinity) → 401 livre
    map[102] = 24;    // getuid
    map[104] = 47;    // getgid
    map[107] = 25;    // geteuid
    map[108] = 48;    // getegid
    map[130] = 130;   // rt_sigtimedwait (livre no OvsbOS)
    map[129] = 129;   // rt_sigqueueinfo (livre no OvsbOS)
    map[134] = 94;    // rt_sigprocmask (134 colide c/ sigaction)
    map[158] = 158;   // arch_prctl
    map[160] = 160;   // setrlimit (livre no OvsbOS)
    map[165] = 165;   // mount (livre no OvsbOS)
    map[166] = 166;   // umount2 (livre no OvsbOS)
    ...
    map[213] = 213;   // epoll_create (livre no OvsbOS)
    map[232] = 232;   // epoll_wait (livre no OvsbOS)
    map[233] = 233;   // epoll_ctl (livre no OvsbOS)
    map[234] = 103;   // tgkill (234 colide c/ nanosleep)
    map[247] = 247;   // waitid (livre no OvsbOS)
    map[218] = 218;   // set_tid_address (218 livre no OvsbOS)
    map[273] = 273;   // set_robust_list (livre no OvsbOS)
    map[274] = 274;   // get_robust_list (livre no OvsbOS)
    map[257] = 257;   // openat (livre no OvsbOS)
    map[258] = 258;   // mkdirat (livre no OvsbOS)
    map[262] = 262;   // newfstatat (livre no OvsbOS)
    map[263] = 263;   // unlinkat (livre no OvsbOS)
    map[264] = 264;   // renameat (livre no OvsbOS)
    map[267] = 267;   // readlinkat (livre no OvsbOS)
    ...
    map[138] = 407;   // fstatfs (138 livre no OvsbOS) → 407
    map[281] = 281;   // epoll_pwait (livre no OvsbOS)
    map[285] = 285;   // fallocate (livre no OvsbOS)
    map[288] = 288;   // accept4 (livre no OvsbOS)
    map[291] = 291;   // epoll_create1 (livre no OvsbOS)
    map[292] = 292;   // dup3 (livre no OvsbOS)
    map[293] = 293;   // pipe2 (livre no OvsbOS)
    map[296] = 296;   // prlimit64 (livre no OvsbOS)
    map[302] = 302;   // prlimit64 (livre no OvsbOS)
    map[318] = 318;   // getrandom (livre no OvsbOS)
    map[271] = 271;   // ppoll (livre no OvsbOS)
    map[283] = 283;   // timerfd_create (livre no OvsbOS)
    map[286] = 286;   // timerfd_settime (livre no OvsbOS)
    map[287] = 287;   // timerfd_gettime (livre no OvsbOS)
    map[290] = 290;   // eventfd2 (livre no OvsbOS)
    map[309] = 309;   // getcpu (livre no OvsbOS)
    map[332] = 332;   // statx (livre no OvsbOS)
    map[326] = 326;   // copy_file_range (livre no OvsbOS)
    map[221] = 221;   // fadvise64 (livre no OvsbOS)
...
//
// OvsbOS C handler expects:
//   regs[4] = a1 (rdi), regs[3] = a2 (rsi), regs[2] = a3 (rdx),
//   regs[1] = a4 (rcx)

// ~~ syscall_handler_zig ~~
// Dispatcher principal de syscalls! Recebe os registradores via IDT,
// traduz o número do syscall (Linux → OvsbOS se necessário),
// ajusta o arg4 (Linux passa em r10, OvsbOS espera em rcx),
...
export fn syscall_handler_zig(regs: [*]u64) void {
    ...
    // Mach-O programs use native OvsbOS syscall numbers. Translate only
    // processes loaded through the Linux ELF compatibility path.
    if (process_current_uses_linux_abi() == 0) {
        syscall_handler(regs);
        return;
    }

    const mapped = linux_to_ovsbos[@as(usize, @intCast(num))];
    regs[0] = @as(u64, mapped);

    // Fix up arg4 for Linux syscalls: Linux puts arg4 in r10 (regs[7]),
    // but OvsbOS C handler expects arg4 in regs[1] (rcx slot).
    // Only do this when the number actually changed (Linux → OvsbOS translation),
    // so OvsbOS-native calls (198-211, or identity-mapped like 186→186) keep rcx.
